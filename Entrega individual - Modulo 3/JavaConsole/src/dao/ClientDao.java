...
public class ClientDao {
	...
	public List<Client> findAll() {
		...
		try {
			...
			while (result.next()) {
				...
				client.setEmail(result.getString(ClientDtoMetadata.email));
				client.setPassword(result.getString(ClientDtoMetadata.password));
				client.setAddress(result.getString(ClientDtoMetadata.address));
				client.setState(result.getString(ClientDtoMetadata.state));
				clients.add(client);
			}
	...
	public List<Client> findByPk(long clientId) {
		...
		sql = String.format(sql, ClientDtoMetadata.tableName, ClientDtoMetadata.id);
	...
	public void update(Client client) {
		sql = "UPDATE %s " + "SET %s = ?, %s = ?, %s = ?, %s = ?, %s = ? " + "WHERE %s = ?";
		sql = String.format(sql, ClientDtoMetadata.tableName, ClientDtoMetadata.fullName, ClientDtoMetadata.email,
				ClientDtoMetadata.password, ClientDtoMetadata.address, ClientDtoMetadata.state, ClientDtoMetadata.id);
